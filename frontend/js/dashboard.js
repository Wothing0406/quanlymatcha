...
async function initDashboard() {
  try {
    ...
    if (gridEl)
      gridEl.innerHTML = `<div class="text-center py-10 text-red-500 font-bold">Khong the ket noi Database. Vui lòng kiem tra lai Bot!</div>`;
  }
}
...
function updateDashboardStats(stats) {
  ...
  if (healthContainer && stats.finance.income > 0) {
    const spentRatio = Math.min(
      (stats.finance.expenses / stats.finance.income) * 100,
      100,
    );
    const colorClass =
      spentRatio > 80
  ...
  const spentData = [
    Math.max(stats.finance.income, 1),
    stats.finance.expenses,
    stats.finance.saving,
  ];
  ...
  const tasksMissed =
    stats.tasks.find((t) => t.status === "skipped")?.count || 0;
  const tasksPostponed =
    stats.tasks.find((t) => t.status === "postponed")?.count || 0;
  const tasksOngoing =
    stats.tasks.find((t) => t.status === "ongoing")?.count || 0;
  const tasksPending =
    stats.tasks.find((t) => t.status === "pending")?.count || 0;
...
async function loadDashboardHistory() {
  ...
  try {
    ...
    listEl.innerHTML =
      activities.length > 0
        ? activities.map((item) => createActivityItemHTML(item, true)).join("")
        : '<div class="text-center py-8 text-gray-400 italic">Chua co hoat dong moi.</div>';
  } catch (err) {
    console.error("Loi tai lich su dashboard:", err);
  }
}
...
async function loadHistory(
  ...
  if (!append)
    gridEl.innerHTML =
      '<div class="col-span-full text-center py-20 opacity-50"><i class="fas fa-spinner fa-spin text-3xl mb-4"></i><p>Dang tim lai ky niem...</p></div>';

  try {
    const activities = await fetchJSON(
      `${API_BASE}/activities?type=${type}&limit=${limit}&offset=${offset}`,
    );

    if (activities.length === 0 && !append) {
      gridEl.innerHTML =
        '<div class="col-span-full text-center py-20 opacity-50 italic">Chua co ky niem.</div>';
      return;
    }
    ...
    if (loadMoreBtn) {
      if (activities.length < limit) loadMoreBtn.classList.add("hidden");
        ...
        loadMoreBtn.onclick = () =>
          loadHistory(type, limit, offset + limit, true);
      }
    }
  } catch (err) {
    console.error("Loi tai lich su:", err);
  }
}
